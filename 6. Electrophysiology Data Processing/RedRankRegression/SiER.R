...
for(rhoo in 1:2)
{
  ...
  for(bo in 1:2)
  {
    ...
    for (i in 1:rep) 
    {
      ...
      # fit cv.SiER
      cv.fit = cv.SiER(x_train,y_train, K.cv=5, upper.comp = r)
      ...
      Y_sier[i] = mean((y_test-pred_y)^2)
      XA_sier[i] = mean((XA-pred_y)^2)
      A_sier[i] = mean((A-Abar)^2)
      TPRs[i] = mean(Abar[1:s,1]!=0)
      FPRs[i] = mean(Abar[(s+1):p,1]!=0)
    }
    results = cbind(Y_sier,XA_sier,A_sier,TPRs,FPRs)
...
write.csv(output, file = "C:/Users/hhilafu/Dropbox/papers/CISE Reduced-Rank/Simulations/SiER_U_n_30_p_100_q_1000.csv")
write.csv(output, file = "/Users/haileabhilafu/Dropbox/papers/CISE Reduced-Rank/Simulations/SiER_t_n_30_p_100_q_1000.csv")
